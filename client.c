...
void longList();
void shortList(char str11[] , char str22[]);
...
int main()
{
	...
	char str1[1124],send_data[1124],recv_data[1124],str2[1124],str3[1124],nameFile[100];
	 memset(hashst,'\0',sizeof(hashst));
					...
					while(token1!=NULL)
					{
						...
						if(strcmp(token1,"IndexGet")==0)
						{
							...
							if(strcmp(token2,"LongList")==0)
							{
								...
								printf("Len of indxgetarray is %d\n",strlen(indexGetArray));
...
void shortList(char str1[],char str2[])
{
	...
	if(d==NULL)
	{
		printf("Could Not read contents.\n");
		return;
	}
	else
	{
